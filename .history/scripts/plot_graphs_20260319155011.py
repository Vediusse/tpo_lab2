...
def plot_system_comparison(frame: pd.DataFrame, png_path: Path, title: str) -> None:
    """Plot the full system with only a zoomed view."""
    x_values = frame["x"].to_numpy(dtype=float)
    approx_values = frame["result"].to_numpy(dtype=float)
    analytical_values = analytical_function("system", x_values)

    plt.figure(figsize=(11, 6))
    plt.plot(x_values, analytical_values, color="#2563eb", linewidth=1.4, label="Analytical")
    plt.plot(x_values, approx_values, color="#0f766e", linewidth=1.8, alpha=0.85, label="Approximation")
...
def plot_system_comparison(frame: pd.DataFrame, png_path: Path, title: str) -> None:
    """Plot the full system with both full-scale and zoomed views."""
    x_values = frame["x"].to_numpy(dtype=float)
    approx_values = frame["result"].to_numpy(dtype=float)
    analytical_values = analytical_function("system", x_values)
    ...
    for axis in axes:
        axis.plot(x_values, analytical_values, color="#2563eb", linewidth=1.4, label="Analytical")
        axis.plot(x_values, approx_values, color="#0f766e", linewidth=1.8, alpha=0.85, label="Approximation")
        axis.grid(True, linestyle="--", alpha=0.5)
        axis.set_ylabel("result")

    axes[0].set_title(f"{title} (full scale)")
    finite = np.concatenate([approx_values, analytical_values])
    finite = finite[np.isfinite(finite)]
    if finite.size:
        full_min = np.min(finite)
        full_max = np.max(finite)
        if full_min < full_max:
            padding = (full_max - full_min) * 0.05
            axes[0].set_ylim(full_min - padding, full_max + padding)

    axes[1].set_title(f"{title} (zoomed)")
    zoom_low, zoom_high = -8.0, 8.0
    axes[1].set_ylim(zoom_low, zoom_high)
    axes[1].set_xlabel("x")
    axes[1].legend()
...
def plot_standard_functions(input_dir: Path, output_dir: Path) -> None:
    ...
    for stem in STANDARD_STEMS:
        ...
        try:
            ...
            plot_comparison(frame, stem, png_path, f"Analytical vs approximation for {stem}")
...
def plot_system_functions(input_dir: Path, output_dir: Path) -> None:
    ...
    for stem in ("system_left", "system_right"):
        ...
        try:
            frame = load_csv(csv_path)
            png_path = output_dir / f"{stem}.png"
            plot_system_comparison(frame, png_path, f"Analytical vs approximation for {stem}")
            print(f"Saved plot: {png_path}")
        except Exception as error:  # noqa: BLE001
            print(f"Failed to plot {csv_path}: {error}")
    ...
    try:
        ...
        plot_system_comparison(frame, png_path, "Analytical vs approximation for system")
        print(f"Saved plot: {png_path}")
    except Exception as error:  # noqa: BLE001
